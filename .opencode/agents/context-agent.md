...
- **Shared References**: `.opencode/references/shared/` 为单一事实源；如需枚举/扫描参考文件，遇到 `<!-- DEPRECATED:` 的文件一律跳过。
...
所有 CLI 调用统一走 Python 脚本。先确认 project_root 正确：
```bash
...
- 从 `state.json` 读取：`progress.current_chapter`、`plot_threads.foreshadowing`（主路径）
- 缺失降级：若 `plot_threads.foreshadowing` 不存在或类型错误，置为空数组并打标 `foreshadowing_data_missing=true`
- 对每条伏笔至少提取：`content`、`planted_chapter`、`target_chapter`、`resolved_chapter`、`status`
- 回收判定：若 `resolved_chapter` 非空，直接视为已回收并排除
- 生成排序键：`remaining = target_chapter - current_chapter`（若缺失则记为 `null`）
